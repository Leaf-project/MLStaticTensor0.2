...
Ts* mkts(size_t deep, size_t *shape, double *arr, size_t *ptrs, float importance, char* label, char type) {
	...
	//
	size_t strlen_ = strlen(label);
	ts->label = malloc(strlen_);
	memcpy(ts->label, label, strlen_);
	//
	ts->type = type;
	return ts;
};
...
Ts* easymkts(size_t deep, size_t *shape, double *arr) {
	return mkts(deep, shape, arr, NULL, 1, "", 0);
};
...
Ts* cpyts(Ts *ts) {
	...
	//
	ret->label = malloc(strlen(ts->label));
	memcpy(ret->label, ts->label, strlen(ts->label));
	//
	ret->type = ts->type;
	return ret;
};
...
Ts* cpytsnull(Ts *ts) {
	double nullarr[ts->len];
	for (size_t i=0; i < ts->len; i++)
		nullarr[i] = 0;
	//
	return mkts(ts->deep, ts->shape, nullarr, ts->pointers, ts->importance, ts->label, ts->type);
};
...
Ts* takefromts(Ts* ts, size_t* from, size_t* to) {
	...
	for (size_t i=0; i < ts->deep; i++)
		shape[i] = to[i] - from[i];
	//
	Ts* ret = mkts(ts->deep, shape, 0, 0, 1, ts->label, 0);
...
Ts* extend(Ts *ts, size_t* ns, double v) {
	...
	//TODO
	Ts* ret = mkts(ts->deep, new_shape, 0, 0, 1, ts->label, 0);	//arr,ptrs,importance,type
	//
