...
bool _same_shapes(Ts *a, Ts *b) {	//	[1,2,3] == [1,2,3]
...
void _inverse_lst(size_t *lst, size_t *out, size_t n) {	// [1,2,3,4,5] => [5,4,3,2,1]
	for (size_t i=0; i < n; i++)
		out[i] = lst[n-i-1];
};//transpose

bool _does_dot_ok(Ts* a, Ts* b) {	// [a0,a1,a2,a3], [b0,b1,b2,b3] a0==b1, a2==b2, a3==b3, b0 is free
	//After deep checking
...
bool _incorrect_kernel(Ts* Kernel) {	//is shape in 2k+1
...
bool _bigger_kernel(Ts* ts, Ts* Kernel) {	//is A bigger than kernel
...
static const char* _lineopstr = "tensor_operations.c:_lineop";
bool _lineop(Ts *Y, Ts *A, Ts *B, Op op) {
	if (_same_shapes(A, B)) {
		for (size_t i=0; i < A->len; i++)
			STSPOS(Y, i) = op(GTSPOS(A, i), GTSPOS(B, i));
		//
	} else if (A->len == 1) {
		double x = GTSPOS(A, 0);
		for (size_t i=0; i < B->len; i++)
			STSPOS(Y, i) = op(x, GTSPOS(B, i));
		//
	} else if (B->len == 1) {
		double x = GTSPOS(B, 0);
		for (size_t i=0; i < A->len; i++)
			STSPOS(Y, i) = op(GTSPOS(A, i), x);
		//
	} else {
		_Msg(ERROR, _lineopstr, "Operator not same shape.");
		return false;
	}
	return true;
};

static const char* _iterstr = "tensor_operations.c:_iter";
bool _iter(Ts *Y, Ts *ts, Op op) {
	if (Y->len != 0) {
		_Msg(ERROR, _iterstr, "Y->len != 0 but have to.");
		return false;
	} else {
		size_t i=0;
		if (op == op_add || op == op_sub) Y->arr[0] = 0.0;
		else if (op == op_mul || op == op_div || op == op_pow) Y->arr[0] = GTSPOS(ts, i++);	//i=0 here
		//Make operations (from i=1)
		for (; i < ts->len; i++) Y->arr[0] = op(Y->arr[0], GTSPOS(ts, i));
		return true;
	}
};

static const char* _transposestr = "tensor_operations.c:_transpose";
bool _transpose(Ts *Y, Ts *ts) {
	#define Deep ts->deep
	size_t i;
	...
	do {
		...
		//Replace
		setv(Y, inversed, ts->arr[realpos(ts, indexs)]);//getv(ts, indexs));
		//
		ptr_indexs = indexspos(ts, ts->pointers[realpos(ts, indexs)]);
		_inverse_lst(ptr_indexs, inv_ptr_indexs, Deep);
		//
		Y->pointers[realpos(Y, inversed)] = realpos(Y, inv_ptr_indexs);
...
static const char* _dotstr = "tensor_operations.c:_dot";
bool _dot(Ts *Y, Ts *A, Ts *B) {
	if (A->deep == 1 && B->deep == 1) {	//Special Vector case
		//Spetial functions
		if (A->shape[0] == B->shape[0]) {
			for (size_t i=0; i < A->len; i++)
				STSPOS(Y,0) += GTSPOS(A,i) * GTSPOS(B,i);
			return true;
		} else {
			_Msg(ERROR, _dotstr, "a->deep == b->deep == 1 but shapes[0] != shapes[1] %li != %li",  A->shape[0], B->shape[1]);
			return false;
		}
	} else if (A->deep != B->deep) {
		_Msg(ERROR, _dotstr, "Not sames deep : %li != %li", A->deep, B->deep);
		return false;
		...
		do {	//For each pixel of ret[index];
			...
			for (i=0; i < A->shape[0]; i++) {
				...
				setv(Y, index, getv(Y, index) + a*b);
			}
		} while (update(index, Y->shape, Deep));
		#undef Deep
		return true;
	} else {
		_Msg(ERROR, _dotstr, "Corret deeps but invalid combinaison of shapes a->shape[0] != b->shape[1] or not sames shapes after 2nd.");
		return false;
	}
};

static const char* _Kconvlstr = "tensor_operations.c:_Kconvl";
bool _Kconvl(Ts *Y, Ts *A, Ts *Kernel, char* mod) {
	#define Deep Y->deep
	if (A->deep != Kernel->deep) {
		_Msg(ERROR, _Kconvlstr, "Not same deeps %li != %li", A->deep, Kernel->deep);
		return false;
	} else if (_incorrect_kernel(Kernel)){
		_Msg(ERROR, _Kconvlstr, "Shapes are written as (2k), but have to be (2k + 1)");
		return false;
	} else if (_bigger_kernel(A, Kernel)) {
		_Msg(ERROR, _Kconvlstr, "Too big kernel");
		return false;
		...
		do {
			...
			do {
				...
				//
				addv(Y, iters, getv(A, A_iters)*getv(Kernel, kernel_iters));
			} while(update(kernel_iters, Kernel->shape, Deep));
			//
		} while (update(iters, Y->shape, Deep));
		//
		return true;
		...
		size_t kernel_iters[Deep];
		//
		size_t extended_iters[Deep];	//Will be recompute each time
		//
		do {
			for (i=0; i < Deep; i++) kernel_iters[i] = 0;
			//
			do {
				for (i=0; i < Deep; i++) extended_iters[i] = iters[i] + kernel_iters[i];
				//
				addv(Y, iters, getv(extended, extended_iters)*getv(Kernel, kernel_iters));
			} while(update(kernel_iters, Kernel->shape, Deep));
			//
		} while (update(iters, Y->shape, Deep));
		//
		...
		_Msg(ERROR, _Kconvlstr, "Mode '%s' not recognised", mod);
		return false;
	}
	#undef Deep
};

static const char* _convlstr = "tensor_operations.c:_convl";
bool _convl(Ts *y, Ts *a, Ts *b, size_t x) {

};

static const char* _poolstr = "tensor_operations.c:_pool";
bool _pool(Ts *Y, Ts *A, PoolFn fn, size_t *block_sizes) {
	size_t i;
	//
	for (i=0; i < A->deep; i++) {
		if (A->shape[i] < block_sizes[i] || A->shape[i] % block_sizes[i] != 0) {
			_Msg(ERROR, _poolstr, "Block bigger than input tensor");
			return false;
		}
	}
	//
	...
	do {
		...
		setv(Y, iters, fn(taked));
		//
		freets(taked);
	} while (update(iters, Y->shape, A->deep));
	return true;
};

static const char* _pyramidalsstr = "tensor_operations.c:_pyramidals";
bool _pyramidals(Ts *y, Ts *a, Ts* W, char* mod, size_t* params) {
...
STs* lineop(Ts *A, Ts *B, Op op) {
	...
	//
	ret->ret = cpyts(A);
	if (_lineop(ret->ret, A, B, op)) {
		ret->isok = true;
		return ret;
	} else {
		_Msg(ERROR, lineopstr, "TraceBack");
		ret->isok = false;
		return ret;
	}
};
...
STs* iter(Ts *ts, Op op) {
	...
	size_t shape[] = {1};
	//
	ret->ret = mkts(1, shape, 0, 0, 1, "", 0);
	//
	if (_iter(ret->ret, ts, op)) {
		ret->isok = true;
		return ret;
	} else {
		ret->isok = false;
...
STs* dot(Ts *A, Ts *B) {
	...
	if (A->deep == 1 && B->deep == 1) {	//Special Vector case
		...
		if (A->shape[0] == B->shape[0]) {
			size_t shape[] = {1};
			ret->ret = mkts(1, shape, 0, 0, 1, "", 0);	//importance set to 1 but not usefull
			_dot(ret->ret, A, B);
		} else {
