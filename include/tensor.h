...
typedef struct {	//Ts[z][y][x] == getv(Ts, {x,y,z})
	...
	size_t deep;
	size_t *shape;			//x,y,z ... (from smallest to biggest container)
	...
	char* label;
	//
	char type;	/*imut = imutable (type, shape, ...)
0 - variable		(Var)				<- imut
1 - meta-variable	(Memory Cell)		<- imut
2 - constant 		(Const)				<- imut
3 - pseudo-constant (Weights)			<- imut
4 - un-used			(Garbadge)			<- imut		the var[t][i] points to nothing , there is no y=...
5 - mutable			(Shape mutable)		ex:encoder decoder
	*/
...
//Memory
Ts* mkts(size_t deep, size_t *shape, double *arr, size_t *ptrs, float importance, char* label, char type);
Ts* easymkts(size_t deep, size_t *shape, double *arr);
Ts* cpyts(Ts *ts);
Ts* cpytsnull(Ts *ts);
bool cpyarr(Ts* dest, Ts* src);
bool freets(Ts *ts);
...
bool _iter(Ts *y, Ts *ts, Op op);			//iter operator	 	: y = (op)iter a    (+,-,*,/,^)
bool _transpose(Ts *y, Ts *ts);				//.T 				: y = a.T
bool _dot(Ts *y, Ts *a, Ts *b);				//Tensor dot		: y = a @ b
bool _Kconvl(Ts *y, Ts *a, Ts *kernel, char* mod);	//Kernel convolution : y = a | kernel ('same':(out.sh == inp.sh), 'valid':no padding)
bool _convl(Ts *y, Ts *a, Ts *b, size_t x);	//Simple convolution : y = a #x b where (2x+1) is number of connections and x level of connections (like size of kernel for Kconvl)
bool _pool(Ts *y, Ts *a, PoolFn fn, size_t *block_sizes);	//max, average, ... : y = pool(a, size:arr)
...
void dKconvl(Ts* dy, Ts* da, Ts* dkernel, char* mod);
